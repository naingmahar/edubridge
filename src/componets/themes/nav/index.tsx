...
export const PublicNavBar = (props:{hideShop?:boolean,hideCart?:boolean}) =>{
  const cartCount = 0;
  // useRecoilValue(getShoppingCartCountState);
  const navigate = useRouter();
  return(
      <div className="fixed top-0 w-screen z-50 bg-white shadow-sm">
        <PublicTopNavBar>
            <div className="flex flex-none justify-center items-center sm:pe-3">
              {!props.hideCart &&<div className="relative">
                <IconButton 
                    className="btn-link" 
                    ...
                    label={ELABELS.empty} 
                    onPress={()=>{
                      if(cartCount)navigate.push("/carts")
                    }} />
