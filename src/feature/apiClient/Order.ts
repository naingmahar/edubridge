...
export const fetchUpdateOrderStatus = (props:{id:number,status:"confirm"|"processing"|"reject"}):Promise<TBasicOrderRes> => {
    return instance.put(END_POINT.order_status+`?id=${props.id}&order_status=${props.status}`)
}
